...
class AutoFilter:
    ...
    def range(self):
        ...
        """
        return self._worksheet._filter_range
    ...
    def range(self, value):
        ...
        """
        if value is None:
            self._worksheet._filter_range = None
            return
        self._worksheet._filter_range = self._worksheet.range(value).address
    ...
    def filters(self):
        ...
        参数：无。
        返回：键为区域内 0-based 列偏移、值为允许文本值元组的字典副本；调用者
        使用 :meth:`set` 和 :meth:`clear` 修改筛选条件。
        """
        return dict(self._worksheet._filter_conditions)

    def set(self, column: int, values) -> "AutoFilter":
        """功能：为区域内指定0-based列偏移设置或替换值筛选。

        使用方法：``ws.auto_filter.set(1, ['已完成'])``。
        参数：``column`` 为相对于筛选区域的 0-based 列偏移；``values`` 为允许值
        可迭代对象，各项按字符串比较。
        返回：当前筛选代理，支持链式调用。
        异常：列索引或值集合无效时抛出 ``TypeError``。
        """
        if isinstance(column, bool) or not isinstance(column, int) or column < 0:
            raise TypeError("column 必须是非负0-based整数")
        if isinstance(values, (str, bytes)):
            raise TypeError("values 必须是非字符串可迭代对象")
        try:
            prepared = tuple(str(value) for value in values)
        except TypeError as error:
            raise TypeError("values 必须是可迭代对象") from error
        self._worksheet._filter_conditions[column] = prepared
        return self
    ...
    def apply(self) -> "AutoFilter":
        ...
        使用方法：先设置 ``range`` 并通过 :meth:`set` 设置条件，再调用
        ...
        """
        if self._worksheet._filter_range is None:
            raise ValueError("应用筛选前必须先设置 auto_filter.range")
        area = self._worksheet.range(self._worksheet._filter_range)
